...
- (void)initBall {
    ...
    [[CCSpriteFrameCache sharedSpriteFrameCache] 
     addSpriteFramesWithFile:@"ChampionSprite.plist"];
    spriteSheet = [CCSpriteBatchNode
                   batchNodeWithFile:@"ChampionSprite.png"];
    ...
    [netImage setPosition:ccp(screenSize.width * 0.8500f, screenSize.height * 0.565f)];
    [self addChild:netImage z:3];
}
-(void)callNextLevel {
    CGSize screenSize = [CCDirector sharedDirector].winSize;
    CCLabelBMFont *champLabel1 = [CCLabelBMFont 
                                  labelWithString:@"You won, YEAH!!!!!"
                                  fntFile:@"gold23.fnt"];
    [champLabel1 setScale:1.7];
    [champLabel1 setPosition:ccp(screenSize.width * .5,
                                 screenSize.height * .65 )];
    [self addChild:champLabel1];
    CCLabelBMFont *champLabel2 = [CCLabelBMFont 
                                  labelWithString:@"Stay tuned for more Jeremy Ball" fntFile:@"gold23.fnt"];
    [champLabel2 setPosition:ccp(screenSize.width * .5,
...
-(void)callOpinion {
        ...
        [CCMoveTo actionWithDuration:.7f 
                            position:ccp(screenSize.width / 2.0f,
                                         screenSize.height * .1f)];
        id moveEffect = [CCEaseIn actionWithAction:move rate:2.0f];
...
- (void)createBallAtLocation:(CGPoint)location {
    ballsprite1 = [[Ball alloc] initWithSpace:space
																		 location:location champplayLayer:self] ;
...
-(void)initializeRounds {
    ...
    self.roundTwo = [NSArray arrayWithObjects: [NSNumber numberWithInt:1],
...
-(void)updateRound {
        ...
        }else
        {
            if (scoreCounter >= 15) {
                ...
                if (!nextLevelShown) {
                    ...
                    id delayTimeAction1 = [CCDelayTime actionWithDuration:.2];
                    
                    id myCallOpinion = [CCCallFunc actionWithTarget:self selector:@selector(callOpinion)];
                    id delayTimeAction2 = [CCDelayTime actionWithDuration:3.0];
                    
                    [self runAction:[CCSequence actions:delayTimeAction1,
                                     myCallFunc,delayTimeAction2,myCallOpinion, nil]]; 
                    nextLevelShown = TRUE;      
                }   
    ...
    if (shot == 1) {
        ...
        // show 15 ft message
        [holdball setPosition:ccp(screenSize.width * 0.16f, screenSize.height * 0.32f)];
...
static cpBool scored (cpArbiter *arb, cpSpace *space, void *ignore) {
    ...
    ChampplayLayer *game = (ChampplayLayer *)groundShape->data;
...
-(void)initLabels {
    ...
    roundLabel = [CCLabelBMFont labelWithString:roundString fntFile:@"gold23.fnt"];
