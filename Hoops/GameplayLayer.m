...
- (void)initBall {
    ...
    [[CCSpriteFrameCache sharedSpriteFrameCache] 
        addSpriteFramesWithFile:@"BasicSprite.plist"];
    spriteSheet = [CCSpriteBatchNode
                                       batchNodeWithFile:@"BasicSprite.png"];
    ...
    [netImage setPosition:ccp(screenSize.width * 0.855f, screenSize.height * 0.5630f)];
...
-(void)callNextLevel {
    CGSize screenSize = [CCDirector sharedDirector].winSize;
    CCLabelBMFont *cityLabel = [CCLabelBMFont 
                                labelWithString:@"You won, tap City Jeremy\n        to go to next level"
                                fntFile:@"red23.fnt"];
    [cityLabel setPosition:ccp(screenSize.width * .475,
                               screenSize.height * .7 )];
    [self addChild:cityLabel];
    
    // Add BigCity Jeremy
    CCSprite *cityLevel = [CCSprite spriteWithSpriteFrameName:@"BigCityJeremy.png"];
    CCMenuItemSprite * cityItem = [CCMenuItemSprite itemFromNormalSprite:cityLevel selectedSprite:nil target:self selector:@selector(callCity)];
    [cityLevel setScale:.75];
    CCMenu *cityMenu = [CCMenu menuWithItems: cityItem, nil];
    [cityMenu setPosition:ccp(screenSize.width * .50, screenSize.height *.4  )];
    [self addChild:cityMenu];
}

- (void)createBallAtLocation:(CGPoint)location {
    
    ballsprite1 = [[Ball alloc] initWithSpace:space location:location gameplayLayer:self];
...
-(void)initializeRounds {
    ...
    self.roundTwo = [NSArray arrayWithObjects: [NSNumber numberWithInt:2],
		...
		self.roundThree =   [NSArray arrayWithObjects: [NSNumber numberWithInt:3],
...
-(void)updateRound {
    ...
    if (currentRound == 4) {
        ...
        }else
        {
            if (scoreCounter >= 4) {
                ...
                // Unlock City layer in NSUserDefaults                
                [[NSUserDefaults standardUserDefaults] setBool:YES forKey:@"city"];
                [[NSUserDefaults standardUserDefaults] synchronize];
                
                // Show option to go to City 
                
                // Set a booie so the sprite doesn't get added again
                if (!nextLevelShown) {
    ...
    if (shot == 1) {
        // Show FT message
        [holdball setPosition:ccp(screenSize.width * 0.30f, screenSize.height * 0.32f)];
        
    } else if (shot == 2) {
        // show 15 ft message
        [holdball setPosition:ccp(screenSize.width * 0.20f, screenSize.height * 0.32f)];
    } else if (shot == 3) {
        // 3pt
        [holdball setPosition:ccp(screenSize.width * 0.11f, screenSize.height * 0.32f)];
    }
...
- (void)ccTouchEnded:(UITouch *)touch withEvent:(UIEvent *)event {
    ...
    jumpStartTime = 0;
    [[GameManager sharedGameManager] playShot];
}
...
- (void)createBoard {
    ...
    shapeBoard3 = cpPolyShapeNew(groundBody, 4, vert1,
																 ccp(winSize.width * .92f, winSize.height * .70f));
...
static cpBool ground (cpArbiter *arb, cpSpace *space, void *data) {
    ...
    else
    {
       ...
       cpSpaceAddPostStepCallback(space,
																	(cpPostStepFunc)postStepRemove, ballShape, data);
    }
    return cpTrue;
}
...
static cpBool scored (cpArbiter *arb, cpSpace *space, void *ignore) {
    ...
    GameplayLayer *game = (GameplayLayer *)groundShape->data;
    
    // Register only when the ball flies from top to bottom
    if (n.y < 0.0f) {
            
        CCLOG(@"Score");            
        //Update scoreCounter and update round
        game->scoreCounter++;
...
- (void)pauseMenu {
    ...
    pauseMenuItem.position = ccp(screenSize.width * .075,screenSize.height * .895);
...
-(void)initLabels {
    ...
    // Sprites for pause, score, goal, and strikeout
    scoreSprite = [CCSprite spriteWithSpriteFrameName:@"ScoreLabel.png"];
    [self addChild:scoreSprite];
    [scoreSprite setPosition:ccp(screenSize.width * 0.195f, screenSize.height * 0.895f)];
    
    goalSprite = [CCSprite spriteWithSpriteFrameName:@"GoalMenu.png"];
    [self addChild:goalSprite];
    [goalSprite setPosition:ccp(screenSize.width * 0.336f, screenSize.height * 0.895f)];
    ...
    [self.scoreLabel setPosition:ccp(screenSize.width *0.195, screenSize.height * .9f)];
    [self addChild:self.scoreLabel];
    
    // Labels for Goal  Fonts
    goalsLabel =  
    [CCLabelTTF labelWithString:@"Test Goal" fontName:@"Helvetica-Bold" fontSize:19];
    [goalsLabel setColor:ccc3(0, 0, 255)];
    [goalsLabel setPosition:ccp(screenSize.width * .336, screenSize.height * .895)];
    [self addChild:goalsLabel];
    
    NSString *roundString = [NSString stringWithFormat:@"Round %i",currentRound]; 
    roundLabel = [CCLabelBMFont labelWithString:roundString fntFile:@"blue23.fnt"];
...
// Help menu, called only once when when the user plays
-(void)initHelpLayer {
    // Create a transparent layer to hold all the labels.
    helpLayer = [(CCLayerColor *)[CCLayerColor alloc] initWithColor:ccc4(200, 200, 200,0)];
    [self addChild:helpLayer];
    
    shotLabel  = [[CCSprite spriteWithFile:@"shotlabel.png"]retain];
    holdLabel  = [[CCSprite spriteWithFile:@"holdlabel.png"]retain];
    shotTypeLabel  = [[CCSprite spriteWithFile:@"shottypelabel.png"]retain];
    scoreCountLabel  = [[CCSprite spriteWithFile:@"scorelabel.png"]retain];
    goalLabel  = [[CCSprite spriteWithFile:@"goallabel.png"]retain];
    champLabel  = [[CCSprite spriteWithFile:@"champlabel.png"]retain];
    letsgoLabel  = [[CCSprite spriteWithFile:@"letsgolabel.png"]retain];
}
...
-(void)addSprites {
    ...
    if (helpCounter == 1) {
        [shotLabel setPosition:ccp(screenSize.width * .5 , screenSize.height * .7)];
        [helpLayer addChild:shotLabel];
         helpCounter = 2;
        
    } else if (helpCounter == 2) {
        [helpLayer removeChild:shotLabel cleanup:YES];
        [holdLabel setPosition:ccp(screenSize.width * .5 , screenSize.height * .7)];
        [helpLayer addChild:holdLabel];
        helpCounter = 3;
        
    } else if (helpCounter == 3) {
        [helpLayer removeChild:holdLabel cleanup:YES];
        [shotTypeLabel setPosition:ccp(screenSize.width * .23, screenSize.height * .65)];
        [helpLayer addChild:shotTypeLabel];
        helpCounter = 4;

    } else if (helpCounter == 4) {
        
        [helpLayer removeChild:shotTypeLabel cleanup:YES];
        [scoreCountLabel setPosition:ccp(screenSize.width * .17 , screenSize.height * .58)];
        [helpLayer addChild:scoreCountLabel];        
        helpCounter = 5;        
        
    } else if (helpCounter == 5) {
        [goalLabel setPosition:ccp(screenSize.width * .52 , screenSize.height * .62)];
        [helpLayer addChild:goalLabel];
        helpCounter = 6;
        
    } else if (helpCounter == 6) {

        [champLabel setPosition:ccp(screenSize.width * .63 , screenSize.height * .82)];
        [helpLayer addChild:champLabel];
        helpCounter = 7;

        
    } else if (helpCounter == 7) {
        [helpLayer removeChild:scoreCountLabel cleanup:YES];
        [helpLayer removeChild:goalLabel cleanup:YES];
        [helpLayer removeChild:champLabel cleanup:YES];

        [letsgoLabel setPosition:ccp(screenSize.width * .5 , screenSize.height * .7)];
        [helpLayer addChild:letsgoLabel];
        helpCounter = 8;
        
    } else if (helpCounter == 8) {
        [self removeChild:helpLayer cleanup:YES];
        
        [helpLayer release];
        
        [shotLabel  release];
        [holdLabel  release];
        [shotTypeLabel release];
        [scoreCountLabel  release];
        [goalLabel  release];
        [champLabel  release];
        [letsgoLabel  release];
    }
...
-(void)helpMenu {
    ...
    id myCallFunc = [CCCallFunc actionWithTarget:self selector:@selector(addSprites)];

    helpCounter = 1;
    id delayTimeAction1 = [CCDelayTime actionWithDuration:.5];
    id delayTimeAction5 = [CCDelayTime actionWithDuration:5];
    id delayTimeAction7 = [CCDelayTime actionWithDuration:7];
    id delayTimeAction10 = [CCDelayTime actionWithDuration:10];
    
    // Only run the help menu once
    NSUserDefaults *ud=[NSUserDefaults standardUserDefaults]; 
    BOOL helpMessageShown = [ud boolForKey:@"helpMessage"];
    ...
    if (!helpMessageShown) {
        ...
        [self runAction:
            [CCSequence actions:
                delayTimeAction1, myCallFunc, delayTimeAction7, myCallFunc, 
                delayTimeAction7, myCallFunc, delayTimeAction7, myCallFunc, 
                delayTimeAction7, myCallFunc, delayTimeAction7, myCallFunc,
                delayTimeAction10, myCallFunc, delayTimeAction5, myCallFunc,
                nil]];
        
        [[NSUserDefaults standardUserDefaults] setBool:YES forKey:@"helpMessage"];
        [[NSUserDefaults standardUserDefaults] synchronize];
     }
}
