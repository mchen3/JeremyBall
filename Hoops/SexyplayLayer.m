...
- (void)initBall {
    ...
    [[CCSpriteFrameCache sharedSpriteFrameCache] 
     addSpriteFramesWithFile:@"SexySprite.plist"];
    spriteSheet = [CCSpriteBatchNode
                   batchNodeWithFile:@"SexySprite.png"];
    ...
    [netImage setPosition:ccp(screenSize.width * 0.8540f, screenSize.height * 0.580f)];
...
-(void)callNextLevel {
    CGSize screenSize = [CCDirector sharedDirector].winSize;
    CCLabelBMFont *champLabel1 = [CCLabelBMFont 
                                  labelWithString:@" THE CHAMP IS HERE!!!"
                                  fntFile:@"gold23.fnt"];
    [champLabel1 setPosition:ccp(screenSize.width * .45,
                                 screenSize.height * .7 )];
    [self addChild:champLabel1];
    
    CCLabelBMFont *champLabel2 = [CCLabelBMFont 
						labelWithString:@"What do you think about this game?\n Go to the options menu and tell me" fntFile:@"gold23.fnt"];
    [champLabel2 setPosition:ccp(screenSize.width * .75,
                                 screenSize.height * .065 )];
    [champLabel2 setScale:.6];
    [self addChild:champLabel2];

    // Add Big Champ Jeremy
    CCSprite *champLevel = [CCSprite spriteWithSpriteFrameName:@"BigChampJeremy.png"];
    CCMenuItemSprite * champItem = [CCMenuItemSprite itemFromNormalSprite:champLevel selectedSprite:nil target:self selector:@selector(callChamp)];
    [champLevel setScale:.75];
    CCMenu *champMenu = [CCMenu menuWithItems: champItem, nil];
    [champMenu setPosition:ccp(screenSize.width * .50, screenSize.height *.4  )];
    [self addChild:champMenu];
}

- (void)createBallAtLocation:(CGPoint)location {
    
    ballsprite1 = [[Ball alloc] initWithSpace:space
																		 location:location sexyplayLayer:self] ;
...
-(void)initializeRounds {
    ...
    // Shot numbers 5,5,7,10
    self.roundOne = [NSArray arrayWithObjects: [NSNumber numberWithInt:3],
 ...
 self.roundThree =  [NSArray arrayWithObjects: [NSNumber numberWithInt:1],
                    [NSNumber numberWithInt:3],[NSNumber numberWithInt:2],
                    [NSNumber numberWithInt:3], [NSNumber numberWithInt:1],
                    [NSNumber numberWithInt:3],[NSNumber numberWithInt:2],
                    nil];
    
self.championRound= [NSArray arrayWithObjects: [NSNumber numberWithInt:3],
                    [NSNumber numberWithInt:2],[NSNumber numberWithInt:1],
    ...
    if (currentRound == 4) {
        ...
        }else
        {
            if (scoreCounter >= 10) {
                ...
                [[NSUserDefaults standardUserDefaults] setBool:YES forKey:@"champ"];
                [[NSUserDefaults standardUserDefaults] synchronize];
                
                
                // Show option to go to Champ
                if (!nextLevelShown) {
...
- (void)createGround {
    ...
    shapeBottomground = cpSegmentShapeNew(groundBody, ccp(0, wins.width * .070f), 
                              ccp(wins.width, wins.width * .070f), .000000001f);
...
- (void)createScoreSensor {
    ...
    shapeScore= cpPolyShapeNew(groundBody, 4, verts,
															 ccp(winSize.width * .88f,winSize.height * .575f));
...
static cpBool scored (cpArbiter *arb, cpSpace *space, void *ignore) {
   ...
   SexyplayLayer *game = (SexyplayLayer *)groundShape->data;
...
-(void)GoalButtonTapped:(id)sender{
        ...
        [goalMenusprite setPosition:ccp(screenSize.width * .74,
...
-(void)PauseButtonTapped:(id)sender
{
    ...
    // Added revision, don't need _pauseScreenUp flag, I removed pauseLayer altogether
   	if(_pauseScreenUp == FALSE)
	   {
        ...
        //Transparent layer
		CGSize screenSize = [[CCDirector sharedDirector] winSize];
		pauseLayer = [CCLayerColor layerWithColor: ccc4(0, 0, 0, 90)
...
-(void)initLabels {
    ...
    roundLabel = [CCLabelBMFont labelWithString:roundString fntFile:@"pink23.fnt"];
