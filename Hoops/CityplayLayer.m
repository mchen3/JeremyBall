...
- (void)initBall {
    ...
    [[CCSpriteFrameCache sharedSpriteFrameCache] 
     addSpriteFramesWithFile:@"CitySprite.plist"];
    spriteSheet = [CCSpriteBatchNode
                   batchNodeWithFile:@"CitySprite.png"];
    ...
    [netImage setPosition:ccp(screenSize.width * 0.8530f, screenSize.height * 0.571f)];
...
-(void)callNextLevel {
    CGSize screenSize = [CCDirector sharedDirector].winSize;
    CCLabelBMFont *zombieLabel = [CCLabelBMFont 
                                  labelWithString:@"You won, tap Zombie Jeremy\n        to go to next level"
                                  fntFile:@"green23.fnt"];
    [zombieLabel setPosition:ccp(screenSize.width * .475,
                                 screenSize.height * .7 )];
    [self addChild:zombieLabel];
    
    // Add Big Zombie Jeremy
    CCSprite *zombieLevel = [CCSprite spriteWithSpriteFrameName:@"BigZombieJeremy.png"];
    CCMenuItemSprite * zombieItem = [CCMenuItemSprite itemFromNormalSprite:zombieLevel selectedSprite:nil target:self selector:@selector(callZombie)];
    [zombieLevel setScale:.75];
    CCMenu *zombieMenu = [CCMenu menuWithItems: zombieItem, nil];
    [zombieMenu setPosition:ccp(screenSize.width * .50, screenSize.height *.4  )];
    [self addChild:zombieMenu];
}

- (void)createBallAtLocation:(CGPoint)location {
    ballsprite1 = [[Ball alloc] initWithSpace:space location:location layer:self] ;
...
-(void)updateRound {
    ...
    if (currentRound == 4) {
        ...
        }else
        {
            if (scoreCounter >= 5) {
                ...
                // Unlock Zombie layer in NSUserDefaults                
                [[NSUserDefaults standardUserDefaults] setBool:YES forKey:@"zombie"];
                [[NSUserDefaults standardUserDefaults] synchronize];
                
                // Show option to go to Zombie
                // Set a booie so the sprite doesn't get called again
                if (!nextLevelShown) {
    ...
    if (shot == 1) {
        ...
        // 3pt
        [holdball setPosition:ccp(screenSize.width * 0.12f, screenSize.height * 0.32f)];
    }
...
- (void)createBoard {
    ...
    shapeBoard1 = cpPolyShapeNew(groundBody, 4, verts, ccp(winSize.width * .82f, winSize.height * .615f));
    shapeBoard1->e = .7;   
    shapeBoard1->u = 0;
    ...
    shapeBoard2 = cpPolyShapeNew(groundBody, 4, vert, ccp(winSize.width * .89f, winSize.height * .615f));
    shapeBoard2->e = .7;
    shapeBoard2->u = 0;
...
static cpBool scored (cpArbiter *arb, cpSpace *space, void *ignore) {
    ...
    CityplayLayer *game = (CityplayLayer *)groundShape->data;
...
-(void)initLabels {
    ...
    [self.scoreLabel setPosition:ccp(screenSize.width *0.182, 
    ...
    roundLabel = [CCLabelBMFont labelWithString:roundString fntFile:@"red23.fnt"];
